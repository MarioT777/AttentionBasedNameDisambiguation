...
lc_center = LMDBClient(LMDB_PAPER_CENTER_EMB)
...
class TripletsGenerator:
    ...
    def getEmbeddingCenters(self, pid):
        return lc_center.get(pid)
    ...
    def embeddings(self, anchorPid, pid_pos, pid_neg):
        if self.getLMDBEmbedding(anchorPid) is not None and  self.getLMDBEmbedding(pid_pos) is not None and self.getLMDBEmbedding(pid_neg) is not None \
            ...
            return self.getAnchorEmbedding(anchorPid), self.getAnchorEmbedding(pid_pos), self.getAnchorEmbedding(pid_neg), self.getLMDBEmbedding(anchorPid), self.getLMDBEmbedding(pid_pos), self.getLMDBEmbedding(pid_neg), self.getEmbeddingCenters(anchorPid)
        else:
            return None, None, None, None, None, None, None

    def testembeddings(self, anchorPid, pid_pos, pid_neg):
        return self.getAnchorEmbedding(anchorPid), self.getAnchorEmbedding(pid_pos), self.getAnchorEmbedding(pid_neg), self.getAnchorEmbedding(anchorPid), self.getAnchorEmbedding(pid_pos), self.getAnchorEmbedding(pid_neg), self.getEmbeddingCenters(anchorPid)
    ...
    def gen_emb_mp(self, task_q, emb_q, role):
        while True:
            ...
            if role == "test":
                emb1, emb_pos, emb_neg, attentionEmb, attentionEmbPos, attentionEmbNeg, emb_center  = self.testembeddings(pid1, pid_pos, pid_neg)
            else:
                emb1, emb_pos, emb_neg, attentionEmb, attentionEmbPos, attentionEmbNeg, emb_center = self.embeddings(pid1, pid_pos, pid_neg)
            # emb1 = lc.get(pid1)
            # emb_pos = lc.get(pid_pos)
            # emb_neg = lc.get(pid_neg)
            if emb1 is not None and emb_pos is not None and emb_neg is not None and attentionEmb is not None and emb_center is not None:
                emb_q.put((emb1, emb_pos, emb_neg, attentionEmb, attentionEmbPos, attentionEmbNeg, emb_center))
        emb_q.put((False, False, False, False, False, False, False))
    ...
    def gen_triplets_mp(self, role='train'):
        ...
        while True:
            ...
            emb1, emb_pos, emb_neg, attentionEmb, attentionEmbPos, attentionEmbNeg, emb_center = emb_q.get()
            if emb1 is False:
                ...
                break
            cnt += 1
            yield (emb1, emb_pos, emb_neg, attentionEmb, attentionEmbPos, attentionEmbNeg, emb_center)
    ...
    def dump_triplets(self, role='train'):
        ...
        for i, t in enumerate(triplets):
            ...
            if len(anchor_embs) == self.batch_size:
                ...
                data_utils.dump_data(emb_centers, out_dir, 'emb_centers_{}_{}.pkl'.format(role, f_idx))
